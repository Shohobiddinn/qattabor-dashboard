            ...
            <UCard v-for="category in categories.data" :key="category.id">
                            ...
                            <template #panel>
                                <div class="flex flex-col gap-1 p-2 w-40">
                                    <UButton icon="i-heroicons-pencil-square" variant="ghost"
                                        @click="openEditModal(category)" class="justify-start">
                                        Tahrirlash
                                    </UButton>
                                    <UButton icon="i-heroicons-trash" color="red" variant="ghost"
                                        @click="deleteCategory(category.slug)" class="justify-start">
                                        O‘chirish
                                    </UButton>
                                </div>
                            </template>
                ...
                <div class="mt-2">
                    <div class="rounded-md w-full h-40 bg-gray-100 flex items-center justify-center overflow-hidden">
                        <img v-if="category.photo" :src="category.photo" alt="Image"
...
async function getCaregory() {
    ...
    const { data, error, refresh } = await request(`/category/all?status=active&page=${categories.value.page}&page_size=${categories.value.page_size}${searchGet}&parent_id=${params.id}`, 'get')
